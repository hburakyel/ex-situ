...
async function updateImageUrl() {
  try {
    let page = 1;
    let totalPages = 1;
    const oldUrlPart = 'https://asset.museum-digital.org//media/800/smb/images/';
    const replacePart = '500w_';
    const newUrlPart = '';
    ...
    do {
      ...
      if (data && data.data.length > 0) {
        for (const entry of data.data) {
          ...
          if (img_url && img_url.startsWith(oldUrlPart) && img_url.includes(replacePart)) {
            const updatedUrl = img_url.replace(replacePart, newUrlPart);
