...
async function publishAllEntries() {
  ...
  try {
    do {
      ...
      if (data && data.data.length > 0) {
        for (const entry of data.data) {
          const { img_url, latitude, longitude, place_name, publishedAt } = entry.attributes;

          // Validate img_url, latitude, longitude, and place_name
          const isValidImgUrl = img_url && img_url.startsWith('http');
          const isValidLatitude = latitude && !isNaN(parseFloat(latitude));
          const isValidLongitude = longitude && !isNaN(parseFloat(longitude));
          const isValidPlaceName = place_name && place_name.trim().length > 0;

          if (isValidImgUrl && isValidLatitude && isValidLongitude && isValidPlaceName && !publishedAt) {
            try {
            ...
            console.log(`Entry with ID: ${entry.id} is invalid or already published.`);
          }
        }
      } else {
    ...
    console.log(`All entries published successfully. Total published: ${publishedCount}, Total errors: ${errorCount}`);
  } catch (error) {
