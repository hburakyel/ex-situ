...
const targetInstitutionName = 'Ethnologisches Museum';
const updatedPlaceName = 'Berlin';

async function fetchEntries(page = 1) {
  try {
    const response = await axios.get(`${strapiBaseUrl}?pagination[page]=${page}&pagination[pageSize]=100&filters[institution_name][$eq]=${targetInstitutionName}`, {
      headers: {
...
async function updateEntry(id, placeName) {
  try {
    ...
    do {
      ...
      if (data && data.data.length > 0) {
        ...
        for (const entry of data.data) {
          ...
          await updateEntry(entry.id, updatedPlaceName);
        }
      } else {
